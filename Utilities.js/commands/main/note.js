    ...
    if (!this.client.notes[message.author.id]) {
      ...
      if (action === 'list') {
        const embed = new RichEmbed()
          ...
          .setColor(0x00CCFF)
          .setTimestamp()
          .setDescription(`To do something to a note do \`${message.guild.commandPrefix}note (action) (note id)\` or, To create a note do \`${message.guild.commandPrefix}note create (note title) (note content)\`\n----------------------`)
      ...
      if (action === 'delete') {
        ...
        message.reply(`To do something to a note do \`${message.guild.commandPrefix}note (action) (note id)\` or, To create a note do \`${message.guild.commandPrefix}note create (note title) (note content)\``)
      }
    }
  }
};
